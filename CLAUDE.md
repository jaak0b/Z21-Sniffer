...
**Meta-rule — no silent omissions.** Whenever a rule says "every X" (every finding, every survivor, every test layer, every key), dropping any X is allowed only with a **logged, evidenced disposition** — silence is itself a violation. Enumerate the X's, and against each record what you did and the proof. "I judged it not worth it" is never a disposition; that trade-off belongs to the owner, not you. This discipline governs all the "every X" rules below (esp. #4, #13, #16) — rule #16 is the template for how it reads in practice.
...
13. **Self-review-and-fix before handoff — multi-file changes only.** Before presenting a multi-file change for commit approval, run a medium-effort `/code-review` scoped to the change. **Every finding it surfaces requires a logged disposition — silence is a violation (see the meta-rule).** Paste the finder list verbatim into the handoff, and against each finding record exactly one of:
    - **Fixed** — name the commit/test that resolves it (each fix follows the TDD and verification rules).
    - **False positive** — the finding is *factually wrong* (the code does not do what it claims), proven with the quoted line that refutes it. The review already adversarially verified every listed finding, so this is rare; you may **not** demote a correct finding to "false positive" because the fix is costly.
    - **Owner-waived** — you flagged it to the owner and they explicitly chose not to fix it. The decision to skip a *correct* finding is the owner's, never yours.

    Severity is irrelevant to whether a finding needs a disposition: a cleanup, altitude, or conventions finding gets the same treatment as a correctness bug. **"Pre-existing," "not mine," "out of scope," "low value," "cosmetic," "risky to change," "mirrors the existing pattern," and "judgment call" are NEVER valid reasons to skip a finding** — exactly as for mutation survivors (rule #16). If you find yourself weighing the fix's cost against the finding's value, stop: that trade-off is the owner's, so it becomes an Owner-waived item, not a silent drop. Single-file changes are exempt, mirroring the verification buckets in rule #5.
...
18. **No new `Func`/`Action` (or other delegate) parameter or field used as a stand-in for a collaborator or callback dependency.** Inject the real collaborator as an interface, or hold the concrete instance and call it — do not wrap it in a delegate handed in at construction. (Concretely: this is exactly why "request the station's current state on Start recording" routes through `ConnectionViewModel`, which already holds the live connection, instead of taking a `Func<Task> requestCurrentState`.) This does **not** restrict ordinary delegate use: LINQ predicates/projections (`Where`, `Select`, …), builder-style `initialize` actions (`GetOrCreate<T>(key, Action<T> initialize)`), event-handler delegate types, and local higher-order helpers (e.g. `OnUi<T>(Func<T>)`) are all fine. The ban is specifically the "a VM/service receives a delegate that wraps a collaborator" pattern. Existing such wiring (`Action<Action> post`, the `Func<Task<string?>>` file pickers, `RecordingViewModel`'s `Func<bool>`/`Func<SystemSnapshot?>`, the callback-based navigation) is **grandfathered** — leave it, but never add more.
19. **Colors live ONLY in the theme files — NEVER hardcode a color anywhere else.** Every color used for rendering — fill, stroke, pen, text — MUST be resolved from the theme palettes (`Themes/Colors.Light.axaml` / `Themes/Colors.Graphite.axaml`) by resource key: in C# through `ThemeBrushResolver` (or `TryFindResource` with the active `ThemeVariant`), in XAML via `{DynamicResource …}`. A color literal in C# or inline in a control — `Color.FromArgb/FromRgb`, `new SolidColorBrush(…)`, `Colors.Gray`, a `#RRGGBB` hex — is forbidden, **including as a "resource missing" fallback**. The single tolerated literal is `Brushes.Transparent` (the absence of paint), used only when a key is genuinely absent. Both palette files must define **every** color key — light and graphite are both first-class — and a fix to one palette must never change a value in the other (verify the untouched palette's keys are byte-identical). New rendering colors get a named key added to both palettes, never a literal at the call site.
...
- [ ] **No new delegate-as-collaborator params** (rule #18) — no new `Func`/`Action` parameter/field wrapping a collaborator; inject the interface or hold the instance instead.
- [ ] **No hardcoded colors** (rule #19) — every rendering color resolved from the theme palettes by key; no color literal in C#/inline (only `Brushes.Transparent` for a missing key); both palettes define every key and the untouched palette is unchanged.
- [ ] **Self-review run and every finding given a logged disposition** (rule #13) — multi-file change: medium `/code-review` on the diff; finder list pasted; each finding marked Fixed / proven-false-positive / owner-waived, with zero silent drops. Single-file change: state the exemption.
...
**Navigation:** `MainWindowViewModel` drives content; `ViewLocator` maps `XxxViewModel → XxxView` by convention. New collaborators are injected as interfaces or held as concrete instances, never passed in as `Action`/`Func` (rule #18). The existing callback-based navigation wiring is grandfathered — don't extend it.
